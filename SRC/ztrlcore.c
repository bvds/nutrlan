...
////
void
ztrlanczos(ztrl_matvec op, trl_info * info, int nrow, int mev, double *eval,
...
//
    ...
    base = (trl_dcomplex *) malloc(nbas * sizeof(trl_dcomplex));
    alpha = (double *) malloc((info->maxlan) * sizeof(double));
    beta = (double *) malloc((info->maxlan) * sizeof(double));
    alfrot = (double *) malloc((info->maxlan) * sizeof(double));
    betrot = (double *) malloc((info->maxlan) * sizeof(double));
    lambda = (double *) malloc((info->maxlan) * sizeof(double));
    res = (double *) malloc((info->maxlan) * sizeof(double));
    yy = (double *) malloc((info->maxlan) * (info->maxlan) *
			   sizeof(double));
    yy2 =
	(trl_dcomplex *) malloc((info->maxlan) * (info->maxlan) *
				sizeof(trl_dcomplex));
    rot =
	(double *) malloc((info->maxlan) * (info->maxlan) *
			  sizeof(double));
    memset(base, 0, nbas * sizeof(trl_dcomplex));
    i2 = 0;
//
	...
	*j1 = *j1 + info->nec;
	...
	if (info->guess <= 0) {
		...
		if (nran < nrow) {
		    ...
		    for (i = 0; i < nrow; i++) {
			evec[j * lde + i].r = drand48();
			evec[j * lde + i].i = drand48();
		    }
