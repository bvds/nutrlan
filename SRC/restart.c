...
////
static void
trl_restart_max_gap_cost_ratio_adjust(int n, int tind, int kept,
				      trl_info * info,
				      double *lambda, double *res,
				      int *kl, int *kr)
{
...
// .. Executable statements ..
    ...
    if (t < 2)
	t = 2;
    if (info->predicted_crate != 0.0) {
	alpha =
	    -log(lambda
		 [info->target_id + (info->old_locked - info->locked)]
		 / (info->old_target)) / info->mgamma;
    } else {
	alpha = 1.0;
    }
    if (t > 2 && t + kept > nd && info->crat > 0.0) {
	min_val = trl_min_gap_ratio_(info, nd, tind, res);
	if (min_val > info->crat)
	    t = max(2, nd - kept - 1);
    }
...
////
static void
trl_restart_min_cost(int nd, int tind, int kept, double *lambda,
		     double *res, trl_info * info, int *kl, int *kr)
{
//
// Purpose
...
//
// merit measure the factor of residual norm reduction
#define cost(i,j) ( exp( - (lambda[(i)]-info->trgt) * abs(j-i) / (lambda[(j)]-info->trgt) ) )
//
// ..
// .. local variables ..
    int i, j, lohi, kli, kri, klm, krm, igap;
