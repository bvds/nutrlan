...
////
void
ztrlan(ztrl_matvec op, trl_info * info, int nrow, int mev, double *eval,
...
// .. executables statements ..
    ...
    info->clk_tot = clk1;
    if (info->ned > mev) {
	printf("info->ned (%d) is larger than mev (%d) reducing info->ned "
	       "to %d\n", info->ned, mev, mev);
	info->ned = mev;
    }
    //
    ...
    ztrlanczos(op, info, nrow, mev, eval, evec, lde, misc, nmis, dwrk, ldwrk);
    //
...
////
void
ztrl_check_ritz(ztrl_matvec op, trl_info * info, int nrow, int ncol,
...
// .. executable statements ..
    ...
    *check = 0;
	...
	gsumwrk = &wrk[0];
	aq = (trl_dcomplex *) malloc(nrow * sizeof(trl_dcomplex));
	if (aq == NULL) {
	    fprintf(stderr,
	...
	aqi = 1;
	gsumwrk = (trl_dcomplex *) malloc(ncol * sizeof(trl_dcomplex));
	if (gsumwrk == NULL) {
	    printf
    ...
    dsumwrk = (double *) malloc(ncol * sizeof(double));
    ...
    res = (double *) malloc(ncol * sizeof(double));
    err = (double *) malloc((2 * ncol) * sizeof(double));
