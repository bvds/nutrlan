...
#include <trlan.h>	/* trl_matvec, trl_info */

/* #define small(tmp, eps) = ( abs(tmp) > (eps)*(anrm) ? (eps)*abs(tmp) : (eps)*(eps)*(anrm) ) */
...
@arg ops     (input) [Functin pointer]
          It points to the function that performs the matrix-vector operation.

@arg info    (input) [Pointer to structure trl_info]
...
@arg eval    (output) [Double array of dimension (mev)]
          On successful exit, contains the eigenvalues.

@arg evec    (output) [Double array of dimension (lde*mev)]
          On successful exit, contains the eigenvectors.

@arg lde     (input) [Integer]
          On entry, specifies the leading dimension of evec.

@arg base    (workspace) [Double precision array of dimension (ldb*nbas)]
          Used to hold the lanczos vectors not fit in evec, i.e.,
          nbas=info->maxlan-mev+1.

@arg ldb     (input) [Integer]
          On entry, specifies the leading dimension of base.  This value
          must not be less then nrow.

@arg nbas    (input) [Integer]
          On entry, specifies the number of columns in base.

@arg wrk     (workspace) [Double precision array of dimension (lwrk)]
...
/*
 Prepare the Ritz values for restarting.
...
@arg nd         (input) [integer]
             It specifies the current size of the Lanczos basis (not locked).

@arg mnd        (input) [integer]
             It specifies the maximum size of the Lanczos basis.
             (lanczos basis size used)-(Ritz values locked).

@arg lambda     (input/output) [double precision array of dimension (nd)]
             It contains the computed Ritz values.

@arg res        (input/output) [double precision array of dimension (nd)]
             It contains the residual norm of the computed Ritz values.

@arg info       (input/output) [pointer to trl_info structure]
             It points to the current trl_info structure.

@arg kept       (input/output) [pointer to integer]
...
@arg info   (input) [Pointer to structure trl_info]
         It points the current TRL_INFO. Used only to pass the maximum counter
         value for the clock.

@arg time   (input/output) [Pointer to integer]
         On entry, contains the processor time spent in one of following
         operations:
...
@arg rtime  (input/output) [Pointer to double]
         On entry, contains the processor time spent in one of following
         operations:
...
// Purpose
// =======
// Print the Ith alpha and beta value to the log file. Function trl_print_real is used.
//
// Arguments
// =========
...
// Purpose
// =======
// Print all computed alpha and beta to the log file. Function trl_print_real is used.
//
// Arguments
// =========
// info     (input) Pointer to structure trl_info_
...
// Purpose
// =======
// Print the lambda and its residual norm computed so far. Function trl_print_real is used.
//
// Arguments
// =========
